...
description: Elixir coding standards and best practices. Use when writing, reviewing, or refactoring Elixir code to ensure idiomatic patterns, proper error handling, and Phoenix conventions.
---
...
```elixir
# ACCEPTABLE — boolean check, if/else is idiomatic here
if admin?(user) do
  grant_access(user)
else
  deny_access(user)
...
# STILL BAD — structural matching shoved into if/else
if user.role == :admin do
  admin_action(user)
else
  regular_action(user)
end
# Use multi-clause functions or case for structural matches
```
